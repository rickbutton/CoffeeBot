...
- **wowaudit upload + verify** — every successful sim (Raidbots **or** QELive) is POSTed to wowaudit's `/v1/wishlists` endpoint (`replace_manual_edits=true`, `clear_conduits=true`). Each upload is bracketed by a `GET /v1/wishlists/{id}` before and after — the job is only stamped with `wowaudit_uploaded_at` if the response body actually changed (wowaudit's POST returns 200 even when the upload silently no-ops, so we treat "body unchanged" as failure). Wowaudit's per-character id is resolved on first use via `GET /v1/characters` (matched on name+realm, case-insensitive, region suffix tolerated) and cached on the `characters` row. Enabled when `WOWAUDIT_API_KEY` is set; the API key alone identifies the team. Reports that pre-date the upload feature can be flushed with `/sim backfill-wowaudit` (one-shot, sequential, idempotent).
...
| `pnpm wowaudit:test <jobId>` | Manually run the wowaudit upload+verify flow for one sim job against the live API. Pair with `pnpm db:pull-prod` to debug specific stuck reports |
